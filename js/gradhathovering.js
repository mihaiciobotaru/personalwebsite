			...
			function init() {
				...
				{					
					ratio = (window.innerWidth) / (window.innerHeight/1.3);
				}else {
				...
				loader.load( 'GraduationHat.gltf', function ( gltf ) {
					if(window.innerWidth > window.innerHeight)	
						gltf.scene.scale.set(0.10,0.10,0.10);
					else gltf.scene.scale.set(0.06,0.06,0.06);
			...
			function onWindowResize() {
				...
				scene.traverse( function ( object ) {
					if(object.type==="Group" & object.name==="Scene"){
						obj = object;
					}
				})
				if(window.innerWidth < window.innerHeight)	
